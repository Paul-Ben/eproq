...
<body
    class="flex items-center justify-center min-h-screen bg-gradient-to-tr from-gray-900 via-gray-800 to-gray-900 text-white">

    <div class="text-center max-w-md p-8 bg-gray-800 bg-opacity-70 rounded-xl shadow-lg">
        <h1 class="text-3xl font-extrabold mb-6 tracking-wide">Enter Your 2FA Code</h1>
        <p class="mb-8 text-gray-400">Please enter the 6-digit code sent to your device.</p>

        <form method="POST" action="{{ route('2fa.store') }}" id="twoFactorForm" class="flex justify-center space-x-4"
            autocomplete="off" onsubmit="return handleSubmit(event)">
            @csrf
            <input type="hidden" name="two_factor_code" id="two_factor_code" />

            <input type="text" inputmode="numeric" maxlength="1" pattern="[0-9]*"
                class="w-14 h-14 text-center text-3xl font-mono bg-gray-900 border border-gray-700 rounded-lg shadow-inner transition focus:ring-4 focus:ring-green-500"
                id="digit-1" />
            <input type="text" inputmode="numeric" maxlength="1" pattern="[0-9]*"
                class="w-14 h-14 text-center text-3xl font-mono bg-gray-900 border border-gray-700 rounded-lg shadow-inner transition focus:ring-4 focus:ring-green-500"
                id="digit-2" />
        ...
        <button type="submit" form="twoFactorForm"
            class="mt-8 w-full py-3 bg-green-600 hover:bg-green-500 rounded-lg font-semibold text-xl tracking-wide transition">
            Verify
        </button>
    ...
    <script>
        ...
        inputs.forEach((input, idx) => {
            ...
            // Optional: allow arrow keys to navigate inputs
            input.addEventListener('keydown', (e) => {
        ...
        //   // TODO: Submit the code to your backend here, e.g., via AJAX or form submit
        //   alert('Code entered: ' + code);

        //   // For demonstration, reset inputs
